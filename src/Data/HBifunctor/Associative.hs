...
retractNE =
  (id !*! biretract @t . hright (retractNE @t))
    . matchNE @t
...
instance Associative CD.Day where
  ...
  consNE (CD.Day x (Div1 xs) f) =
    Div1 . runNonEmptyF . consNE $
      CCY.Coyoneda (fst . f) x :*: contramap (snd . f) (NonEmptyF xs)
  toNonEmptyBy (CD.Day x y f) =
    Div1 . runNonEmptyF . toNonEmptyBy $
...
appendNEIDay_ (ID.Day xs ys g f) = case xs of
  ...
  More1 (ID.Day z zs j h) ->
    More1 $
      ID.Day
        z
        (appendNEIDay_ (ID.Day zs ys (,) id))
        (\a (b, c) -> g (j a b) c)
        (B.assoc . first h . f)
...
appendNEINight_ (IN.Night xs ys f g h) = case xs of
  ...
  More1 (IN.Night z zs j k l) ->
    More1 $
      IN.Night
        z
        (appendNEINight_ (IN.Night zs ys Left Right id))
        (f . j)
        (either (f . k) g)
        (B.assoc . first l . h)
...
deriving via
  (WrappedHBifunctor (WrapHBF t) f)
  instance
    HBifunctor t => HFunctor (WrapHBF t f)
...
instance Associative t => Associative (WrapHBF t) where
  ...
  associating =
    isoF (hright unwrapHBF . unwrapHBF) (WrapHBF . hright WrapHBF)
      . associating @t
